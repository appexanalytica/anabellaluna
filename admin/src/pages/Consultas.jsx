...
const Consultas = () => {
  ...
  // Property inquiries + site contact form state
  ...
  const focusKey = params.get('id') ? `${params.get('origen') || 'activity'}-${params.get('id')}` : '';
  ...
  const filteredItems = useMemo(() => {
    ...
    if (filter === 'contacto') result = result.filter((r) => r.origen === 'contacto');
  ...
  const reload = useCallback(async () => {
    ...
    setError('');
    try {
      const [enquiries, visits, contactos] = await Promise.all([
      ...
      const merged = []
        ...
        .concat(Array.isArray(contactos) ? contactos.map(mapContacto) : []);
  ...
  // Al llegar desde el dropdown, resaltar y hacer scroll a la consulta elegida
  useEffect(() => {
    if (!focusKey || items.length === 0) return;
    const node = rowRefs.current[focusKey];
    if (node) node.scrollIntoView({ behavior: 'smooth', block: 'center' });
  }, [focusKey, items]);

  const markReadApi = (row, read) => (
    row.origen === 'contacto'
      ? crmService.contactMessages.markRead(row.id, read)
      : crmService.activities.markRead(row.id, read)
  );

  const toggleRead = async (row) => {
    const next = !row.leida;
    try {
      await markReadApi(row, next);
      setItems((prev) => prev.map((r) => (r.key === row.key ? { ...r, leida: next } : r)));
    } catch (e) {
  ...
  // Estilo por tipo de consulta (visita / contacto / consulta de propiedad)
  const rowStyle = (row) => {
    if (row.type === 'visit_scheduled') {
      return {
        label: 'Solicitud de Visita',
        icon: <FaCalendarCheck className="text-white" />,
        header: currentMode === 'Dark' ? 'bg-green-900/30 border-b border-green-800' : 'bg-green-50 border-b border-green-200',
        badge: currentMode === 'Dark' ? 'bg-green-800' : 'bg-green-500',
    ...
    if (row.origen === 'contacto') {
      return {
        ...
        header: currentMode === 'Dark' ? 'bg-amber-900/30 border-b border-amber-800' : 'bg-amber-50 border-b border-amber-200',
        badge: currentMode === 'Dark' ? 'bg-amber-800' : 'bg-amber-500',
    ...
    return {
      ...
      header: currentMode === 'Dark' ? 'bg-purple-900/30 border-b border-purple-800' : 'bg-purple-50 border-b border-purple-200',
      badge: currentMode === 'Dark' ? 'bg-purple-800' : 'bg-purple-500',
  ...
  const statCards = [
    ...
    { key: 'contacto', label: 'Formulario de Contacto', value: stats.contacto, ring: 'ring-amber-500', bg: 'from-amber-50 to-amber-100', icon: <FaEnvelopeOpenText className="text-xl text-white" />, iconBg: currentMode === 'Dark' ? 'bg-amber-900/50' : 'bg-amber-500', valueColor: 'text-amber-600 dark:text-amber-400' },
  ];
    ...
    <div className={`min-h-screen px-6 lg:px-8 pt-4 pb-6 ${currentMode === 'Dark' ? 'bg-main-dark-bg' : 'bg-gray-50'}`}>
      <div className="mb-6">
        ...
        <p className={`text-sm mt-1 ${currentMode === 'Dark' ? 'text-gray-400' : 'text-gray-500'}`}>
          Consultas de propiedades y mensajes del formulario de contacto del sitio
        </p>
      </div>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-6">
        <div className={`flex-1 max-w-md relative ${currentMode === 'Dark' ? 'text-gray-100' : ''}`}>
          ...
          <input
            type="text"
            placeholder="Buscar por nombre, email, asunto, propiedad..."
            value={searchTerm}
          ...
          return (
            <div
              ...
              className={`rounded-xl overflow-hidden transition-all hover:shadow-lg ${
                focusKey === row.key ? 'ring-2 ring-blue-500' : ''
            ...
            >
              {/* Card Header */}
              <div className={`px-5 py-3 flex items-center justify-between ${style.header}`}>
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-lg ${style.badge}`}>{style.icon}</div>
              ...
              <div className="p-5">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      ...
                      <div>
                        ...
                        {row.contact.email && (
                          <p className="text-sm text-gray-500 dark:text-gray-400 flex items-center gap-1">
                            ...
                            <a href={`mailto:${row.contact.email}?subject=${encodeURIComponent(row.asunto ? `Re: ${row.asunto}` : `Re: Consulta sobre ${row.property.title || 'propiedad'}`)}`} className="hover:underline">
                              {row.contact.email}
                            </a>
                          </p>
                        )}
                  ...
                  <div className="space-y-3">
                    <h4 className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">
                      {row.origen === 'contacto' ? 'Asunto' : 'Propiedad'}
                    </h4>
                    {row.origen === 'contacto' ? (
                      row.asunto ? (
                        <p className="font-medium text-gray-900 dark:text-gray-100">{row.asunto}</p>
                      ) : (
                        <p className="text-sm text-gray-400 dark:text-gray-500 italic">Sin asunto</p>
                      )
                    ) : row.property.title ? (
                      <div className="flex items-center gap-3">
