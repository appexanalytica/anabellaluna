...
import { FaUsers, FaRegCalendarAlt, FaDollarSign, FaChartBar, FaBuilding, FaUserTie, FaFileAlt, FaMagic, FaImage, FaBalanceScale, FaRobot, FaCog, FaVrCardboard, FaEye, FaEnvelope, FaGift, FaComments } from 'react-icons/fa';
...
const menuItems = [
  ...
  { name: 'Consultas', path: '/consultas', icon: <FaEnvelope /> },
      ...
      >
        ...
        <div className="p-3 border-t border-gray-700 transition-all duration-300">
          <button
            ...
            title={`Novedades · Version generada con ${APP_COMMIT_COUNT} commits`}
            className={`
          ...
          >
            ...
            <span className={`flex flex-col items-start whitespace-nowrap transition-all duration-300 ${isExpanded ? 'opacity-100' : 'opacity-0 w-0 overflow-hidden'}`}>
              ...
              <span className="text-[11px] text-gray-500 leading-tight">{`ERP ${APP_VERSION}`}</span>
            </span>
          </button>
        </div>
      </div>
