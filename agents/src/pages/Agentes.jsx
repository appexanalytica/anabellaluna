...
import { FaUserPlus, FaUser, FaStar, FaUsers, FaDollarSign, FaHome, FaMapMarkerAlt, FaShieldAlt, FaTimes, FaSave, FaThLarge, FaEdit, FaTrash, FaPhone, FaEnvelope, FaCalendar, FaChartLine, FaTrophy, FaBriefcase, FaExclamationTriangle, FaKey, FaCopy } from 'react-icons/fa';
...
const Agentes = () => {
  ...
  const avgSatisfaccion = agentes.length > 0 ? Math.round(agentes.reduce((s, a) => s + a.satisfaccionCliente, 0) / agentes.length) : 0;
  const avgPropiedades = agentes.length > 0 ? Math.round(totalPropiedades / agentes.length) : 0;
  const avgComisiones = agentes.length > 0 ? Math.round(totalComisiones / agentes.length) : 0;
  const maxPropiedades = agentes.length > 0 ? Math.max(...agentes.map(a => a.propiedades)) : 0;
  const maxComisiones = agentes.length > 0 ? Math.max(...agentes.map(a => a.comisiones)) : 0;
  const maxPropiedadesScale = Math.max(1, maxPropiedades);
  const maxComisionesScale = Math.max(1, maxComisiones);
  const maxRating = agentes.length > 0 ? Math.max(...agentes.map(a => a.rating)) : 0;
  const comisionPorPropiedad = totalPropiedades > 0 ? Math.round(totalComisiones / totalPropiedades) : 0;
      ...
      pie: {
        donut: {
          ...
          labels: {
            ...
            total: { show: true, label: 'Promedio', fontSize: '11px', color: currentMode === 'Dark' ? '#9CA3AF' : '#6B7280', formatter: () => `${avgSatisfaccion}%` },
          },
        },
      },
    },
                      ...
                      <div className="text-right">
                        ...
                        <div className="mt-2 w-32 bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                          <div 
                            ...
                            style={{ width: `${(agente.propiedades / maxPropiedadesScale) * 100}%` }}
                          />
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right ml-4">
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                          ${Math.round(agente.propiedades > 0 ? agente.comisiones / agente.propiedades : 0).toLocaleString()} por propiedad
                        </p>
                        <div className="mt-2 w-40 bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                          <div 
                            className="bg-purple-600 h-2 rounded-full" 
                            style={{ width: `${(agente.comisiones / maxComisionesScale) * 100}%` }}
                          />
                        </div>
                      </div>
                    </div>
